...
public class Blackjack {
    ...
    public static void main(String[] args) throws Exception {
        ...
        List<Integer> playerDeck = new ArrayList<>();	//creates deck for player
        List<Integer> dealerDeck = new ArrayList<>(); //creates deck for dealer
        
        //giving player starting 2 cards
        playerDeck.add(draw());
        playerDeck.add(draw());
        dealerDeck.add(draw());
        dealerDeck.add(draw());
        ...
        // Player's turn
        while (true) {
            String action = playerChoice();		//prompt player for their action
            if (action.equals("stand") || action.equals("stop") || action.equals("pass"))	//breaks the loop if the player stands
                break;
            else if (action.equals("hit"))
                playerDeck.add(draw());		//adds a card to the players deck
            	displayPlayerCards(playerDeck, true);	//displays the players cards using displayPlayerCards()
            
            // Check if player busted
            int playerTotal = calculateTotal(playerDeck);	//checks sum of cards in deck using calculateTotal()
            if (playerTotal > 21) {
                System.out.println("You have bust!");
                displayPlayerCards(dealerDeck,false);
        ...
        // Dealer's turn
        while (calculateTotal(dealerDeck) < 17)	{
        	System.out.println("The dealer hits.");
        	dealerDeck.add(draw());
        	if (calculateTotal(dealerDeck) > 21) {
                System.out.println("The dealer has bust!");
                System.out.println("You win!");
                return;
            }
        	else if (calculateTotal(dealerDeck) >= 17) {
        		System.out.println("The dealer stands.");
        	}
        }	

        // Compare hands
        displayPlayerCards(playerDeck, true);
        displayPlayerCards(dealerDeck, false);
        if (calculateTotal(playerDeck) > calculateTotal(dealerDeck))
            System.out.println("You win!");
        else if (calculateTotal(playerDeck) < calculateTotal(dealerDeck))
            System.out.println("Dealer wins!");
        else
    ...
    public static int draw() {
        // Generate a random card from 2 to 10
    ...
    public static int calculateTotal(List<Integer> deck) {
        int total = 0;
        for (int card : deck)
            total += card;
        return total;
    ...
    public static void displayPlayerCards(List<Integer> playerDeck, boolean isPlayer) {
    	//HOUSE = true, PLAYER = false
    	StringBuilder cardsString = new StringBuilder();
    	
    	//checks what string to append based on if it is the players deck its reading or the dealers deck.
    	if (isPlayer == true) {
    		cardsString.append("Your cards are: ");
    	}
    	else {
    		cardsString.append("The dealer's cards are: ");
    	}

        for (int i = 0; i < playerDeck.size(); i++) {
            cardsString.append(playerDeck.get(i));
            
            if(i == playerDeck.size() - 2) {
            	cardsString.append(" and ");
            } 
            else if (i != playerDeck.size() - 1){
            	cardsString.append(", ");
            }
        }
